...
public class Blueprint {
    ...
    public static void generate_build () {
        ...
        ArrayList<BlockPos> allPavementBlueprint = getAllPavementPlacements();

        if (Session.buildMode == Enum.buildMode.PAVE) {

            for (BlockPos pos : allPavementBlueprint) {

                if (mc.player.world.getBlockState(pos).getBlock().equals(material)) {
                    put_to_blueprint(blueprints.full_blueprint, pos, material);
                } else {
                    put_to_blueprint(blueprints.blueprint, pos, material);
                }
        ...
        ArrayList<BlockPos> allClearingBlueprint = getAllClearingPlacements();

        for (BlockPos pos : allClearingBlueprint) {
    ...
    public static boolean getDigAtBlock(BlockPos pos, ArrayList<BlockPos> allPavementPlacements) {

        if (mc.player.world.getBlockState(pos).getBlock() == Block.getBlockById(0)) {
            return false;
        }

        if (!allPavementPlacements.contains(pos)) {
            return true;
        }

        return !(mc.player.world.getBlockState(pos).getBlock() == Block.getBlockFromName(Config.config.get("material")));
    ...
    public static ArrayList<BlockPos> getAllPavementPlacements() {

        ArrayList<BlockPos> blueprintReturn = new ArrayList<BlockPos>();
        ...
        if (Session.direction == Enum.direction.PX || Session.direction == Enum.direction.NX || Session.direction == Enum.direction.PZ || Session.direction == Enum.direction.NZ) {
            ...
            for (int i = axis_progress; i < axis_progress + 4; i++) {
        ...
        // DIAGONALS PAVING
            ...
            blueprintReturn.addAll(positions);
    ...
    public static ArrayList<BlockPos> getAllClearingPlacements() {

        ArrayList<BlockPos> blueprintReturn = new ArrayList<>();

        Block material = Block.getBlockFromName(Config.config.get("material"));
        int width = Integer.parseInt(Config.config.get("width"));
        int height = Integer.parseInt(Config.config.get("height"));
        ArrayList<BlockPos> allPavementPlacements = getAllPavementPlacements();
        ...
        if (Session.direction == Enum.direction.PX || Session.direction == Enum.direction.NX || Session.direction == Enum.direction.PZ || Session.direction == Enum.direction.NZ) {
            ...
            if (Session.direction == Enum.direction.PX || Session.direction == Enum.direction.NX) {
                ...
                for (int i = 0; i < onsets.size(); i++) {
                    ...
                    if (getDigAtBlock(position, allPavementPlacements)) {
                        blueprintReturn.add(position);
                    }
